    ...
    respond_to do |format|
      if @enquiry.save
        begin
          ...
          format.html { redirect_to(thank_you_enquiries_path, :notice => 'Enquiry successfuly sent.') }
          format.xml  { render :xml => @enquiry, :status => :created, :location => @enquiry }
        rescue Exception => e
          format.html { redirect_to(@enquiry, :notice => "Cannot send email. Error is #{e.message}") }
          format.xml  { render :xml => @enquiry, :status => :created, :location => @enquiry }
        end
